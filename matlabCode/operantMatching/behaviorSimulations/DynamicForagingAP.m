classdef DynamicForagingAP
    ...
    %
    % adapted from BAB 6/21/17 DynamicForaging code by Helia on 03/06/2020 
    %
    properties
    ...
    methods
        function obj = DynamicForagingAP(varargin) % constructor            
            p = inputParser;
            
            % default parameters if none given
            p.addParameter('RewardProbabilities', [10 90]);
            p.addParameter('BlockLength', [20 35]);
            p.addParameter('stateLength', [40 100]);
            p.addParameters('stateType', [0 1]);
            ...
            obj.stateLength = p.Results.stateLength;
            ...
            obj.stateEnd = randi([min(obj.stateLength) max(obj.stateLength)]) + obj.Trial;
            obj.BlockSwitch = [obj.BlockSwitch obj.BlockEnd];
            obj.stateSwitch = [obj.stateSwitch obj.stateEnd];
            
        function obj = inputChoice(obj, currChoice)
            % increment trial
            obj.Trial = obj.Trial + 1;
            if obj.nextState.flag == true
                obj.stateEnd =  randi([min(obj.stateLength) max(obj.stateLength)]) + obj.Trial;
                obj.stateSwitch = [obj.stateSwitch obj.stateEnd];
                obj.statetype = obj.stateType(2:-1:1);
                obj.newState_flag = false;
            end
            ...
            % next block
            if obj.Trial == obj.stateSwitch(end)
                obj.newState_flag = true;
