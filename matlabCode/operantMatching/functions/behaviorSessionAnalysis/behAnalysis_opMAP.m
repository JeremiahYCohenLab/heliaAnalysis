...
if isstrprop(sessionName(end), 'alpha')
    ...
    behSessionDataPath = [root animalName sep sessionFolder sep 'sortedap' sep 'session' sep sessionName '_sessionData_behav.mat'];
end
...
% hold on;
% str = '\ast';
% text(0,1.2,str,'Interpreter','tex')
...
xlabel('Trials')
ylabel('<-- Left       Right -->')
h = legend('Choices','Rewards');
set(h, 'FontSize',3);
xlim([1 length(allChoice_R)])
ylim([-1 1])
hold on;
% str = 'safe';
% annotation(f,'textbox', [0.2 0.74 0.25 0.15], 'safe','test outside the axes', 'FaceColor', [255,240,245]./255)
% % set(text,'FontSize',8,'color', [255,240,245]./255);
% hold on;
% str = 'under threat';
% text(-3,2.12, str,'color',[255,255,0]./255); hold on;
...
allNoRewards(allRewards~=0) = 0;  %%gives value of 0 to trials w
noRwdMatx = [];
for i = 1:tMax
