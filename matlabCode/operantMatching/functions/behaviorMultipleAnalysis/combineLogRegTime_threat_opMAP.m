...
%     end
    ...
    while (stateSwitch == true)
        ...
        if( h -d >=13)
            allReward_R = [behSessionData(responseInds_threat(d:h)).rewardR]; 
            allReward_L = [behSessionData(responseInds_threat(d:h)).rewardL]; 
            allChoices = NaN(1,length(behSessionData(responseInds_threat(d:h))));
        ...
        rwdTmpMatx = NaN(tMax, length(responseInds_threat(d:h)));     %initialize matrices for number of response trials x number of time bins
        noRwdTmpMatx = NaN(tMax, length(responseInds_threat(d:h)));
        for j = 2:length(responseInds_threat(d:h))          
            k = 1;
    ...
    %concatenate temp matrix with combined matrix
     rwdTmpMatx(:,1) = NaN;
     rwdMatx_states = [rwdMatx_states NaN(length(timeBinEdges)-1, 13) rwdTmpMatx]; %these are the matrices of each state
     noRwdMatx_states = [noRwdMatx_states NaN(length(timeBinEdges)-1, 13) noRwdTmpMatx];
