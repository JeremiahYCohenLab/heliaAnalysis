function [glm_rwdLickThreat, threatStayLickLat, threatSwitchLickLat, binSize, timeMax] = combineLogRegLickLatThreatTime_opMAP(xlFile, animal, category, varargin)
...
rwdTmpMatx = zeros(timeMax, length(responseInds_threat));  
  ...
  %fill in NaNs at beginning of session
    j = 2;
    while behSessionData(responseInds_threat(j)).rewardTime - behSessionData(responseInds(1)).rewardTime < timeMax
        tmpDiff = behSessionData(responseInds_threat(j)).rewardTime - behSessionData(responseInds_threat(1)).rewardTime;
...
glm_rwdLickThreat = fitglm([rwdMatx]', combinedLickLat);
