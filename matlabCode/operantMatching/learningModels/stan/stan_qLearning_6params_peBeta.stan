...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    betaRate[i] = Phi_approx(mu_p[4] + sigma[4] * betaRate_pr[i]) ;
    betaMin[i] = Phi_approx(mu_p[5] + sigma[5] * betaMin_pr[i]) * 20;
    betaMax[i] = Phi_approx(mu_p[6] + sigma[6] * betaMax_pr[i]) * 20;
  }
}
model {
  // Hyperparameters
...
generated quantities {
  ...
  mu_betaRate = Phi_approx(mu_p[4]); 
  mu_betaMin = Phi_approx(mu_p[5]) * 20;
  mu_betaMax = Phi_approx(mu_p[6]) * 20;
