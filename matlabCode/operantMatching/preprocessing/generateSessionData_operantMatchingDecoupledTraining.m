function [behSessionData, blockSwitch, blockSwitchL, blockSwitchR] = generateSessionData_operantMatchingDecoupledTraining(sessionName)
...
behavioralDataPath = [root animalName sep sessionFolder sep 'behavior' sep sessionName '.asc']; %filepath = [root animalName sep sessionFolder sep 'behavior' sep sessionName '.asc'];

% if isstrprop(sessionName(end), 'alpha')
%     behavioralDataPath = [root animalName sep sessionFolder sep 'sortedTraining' sep 'session ' sessionName(end) sep];
% else
%     behavioralDataPath = [root animalName sep sessionFolder sep 'sortedTraining' sep 'session' sep];
% end

sessionText = importData_operantMatching(behavioralDataPath);

behSessionData.trialType = [];
behSessionData.stateType = []; 
...
behSessionData.lickAfterpuff = [];
behSessionData.autopause = [];
behSessionData.delayNlw = [];
behSessionData.allLicks = [];
behSessionData.ManulWaterL = [];
behSessionData.ManulWaterR = [];
            ...
            if (~waterDeliverFlag)
             ...
             if (~DoneManual)
                if strfind(sessionText{currTrialInd},'L port - Manual Water Delivered') 
                       temp = regexp(sessionText(currTrialInd), ': ', 'split');
                       behSessionData(currTrial).ManulWaterL = str2double(temp{1}{2});
                       DoneManual = true;
                end
                if strfind(sessionText{currTrialInd},'R port - Manual Water Delivered')
                        temp = regexp(sessionText(currTrialInd), ': ', 'split');
                        behSessionData(currTrial).ManulWaterR = str2double(temp{1}{2});
                        DoneManual = true;
                end
            end
...
%                 end
                ...
                if ~DoneManual 
                    behSessionData(currTrial).ManulWaterL = NaN;
                    behSessionData(currTrial).ManulWaterR = NaN;
                end
        ...
        if ~isempty(strfind(sessionText{currTrialInd},'Delayed'))
            temp = regexp(sessionText(currTrialInd), ': ', 'split');
            behSessionData(currTrial).delayNlw = str2double(temp{1}{2});
        end
      end
    end
end
