function [Q,ets,numFinishes,Model_ns,Model_nr,cr] = dynaQ_maze(alpha,epsilon,gamma,nPlanningSteps,s_start,s_end,MZ, MAX_N_STEPS)
...
%   end
 ...
 % MZ = mz_fn(tsi);             % <- get the current maze for this timestep 
  ...
  for pi=1:nPlanningSteps
    ...
    r_ati = randi(length(act_taken(r_sti,:)));
